...
const CardA = ({ data, color }) => {
  // const titleRouteMain = `${data.Title} ${data._id}`.replaceAll(" ", "-");
  const titleRouteMain = `${titleToSlug(data.Title)}-${data._id}`;
  return (
    <Flex
      background={{
        base: `${color}`,
        lg: "rgba(0, 0, 0, 0) none repeat scroll 0% 0%",
      }}
      pt={{ base: "31px", lg: "0px" }}
      pb={{ base: "38px", lg: "0px" }}
      mb={{ base: "15px", lg: "0px" }}
    >
      ...
      >
        <Flex
          ...
          width={{ base: "calc(248% / 5.20)", lg: "calc(47.6923%)" }}
          height={"auto"}
          flex={{ base: "0 0 calc(248% / 5.20)", lg: "0 0 calc(47.6923%)" }}
          paddingTop={{ base: "calc(270% / 5.2)", lg: "calc(51.9231%)" }}
        >
          <Image
